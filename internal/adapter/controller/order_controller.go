...
func (c *OrderController) PrintOrderReceipt(ctx *fiber.Ctx) error {
	orderIDParam := ctx.Params("id")
	if orderIDParam == "" {
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorResponse{
			Status:  fiber.StatusBadRequest,
	...
	err = c.orderUseCase.PrintOrderReceipt(ctx.Context(), orderID)
	if err != nil {
		return HandleError(ctx, err, c.errorPresenter)
	}
...
func (c *OrderController) PrintOrderQRCode(ctx *fiber.Ctx) error {
	orderIDParam := ctx.Params("id")
	if orderIDParam == "" {
		return ctx.Status(fiber.StatusBadRequest).JSON(ErrorResponse{
			Status:  fiber.StatusBadRequest,
	...
	err = c.orderUseCase.PrintOrderQRCode(ctx.Context(), orderID)
	if err != nil {
		return HandleError(ctx, err, c.errorPresenter)
	}
